...
Player = Mage(1000,100,100,200,200,9000,100,100,400,400)
Enemy = Dragon(100000,2500,100,900000,2000,400)
...
while Play:
    if Player.Hp <= 0:
        ...
        PlayerTurn = True
        while PlayerTurn:
            n = int(input("\nIt is your turn. \nPress 1 to view move list, Press 2 to view current stats, Press 3 to Attack , 4 to exit: "))
            if n == 1:
                for i in range(1,11):
                    print(f"Move {i}")
                    for j in range(0,6):
        ...
        if PlayerMove["Atk shred"] > 0:
            print(f"You just shred the dragon's atk by {PlayerMove["Atk shred"]}.\n")

        if PlayerMove["Def shred"] > 0:
            print(f"You shredded the dragons defense by {PlayerMove["Def shred"]}.\n")
        Enemy.Atk = Enemy.Atk - (PlayerMove["Atk shred"])
        Enemy.Def = Enemy.Def - (PlayerMove["Def shred"])
        #Player Buffs
        if PlayerMove["heal"] > 0:
            print(f"You are healed for {PlayerMove["heal"]}\n")

        if PlayerMove["Atk Buff"] > 0:
            print(f"You recieved an atk buff of {PlayerMove["Atk Buff"]}\n")

        if PlayerMove["Def Buff"] > 0:
            print(f"You recieved a defense buff of {PlayerMove["Def Buff"]}\n")

        if PlayerMove["Int Buff"] > 0:
            print(f"You gained wisdom. +{PlayerMove["Int Buff"]} Int.\n")

        if PlayerMove["Mana Buff"] > 0: 
            print(f"You gained a mana buff of {PlayerMove["Mana Buff"]}\n")
        ...
        if DragMove["Atk shred"] > 0:
            print(f"The dragon just shredded your attack by {DragMove["Atk shred"]}.\n")

        if DragMove["Def shred"] > 0:
            print(f"The dragon just shredded your defense by {DragMove["Def shred"]}. \n")

        if DragMove["Int shred"] > 0:
            print(f"The dragon has intimidated you, reducing your intelligence by {DragMove["Int shred"]}. \n")
        ...
        if DragMove["heal"] > 0:
            print(f"The dragon healed it self for: {DragMove["heal"]}.\n")

        if DragMove["Atk Buff"] > 0:
            print(f"The dragon buffed its attack by {DragMove["Atk Buff"]}.\n")

        if DragMove["Def Buff"] > 0:
            print(f"The Dragon buffed it's defense by {DragMove["Def Buff"]}.\n")
